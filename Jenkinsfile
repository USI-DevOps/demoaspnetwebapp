  ...
  stages {
    ...
    stage('Checkout') {
      steps {
        ...
        git credentialsId: 'GithubJenkins',
        url: 'https://github.com/USI-DevOps/demoaspnetwebapp.git',
        branch: 'master'
      }
    }
    ...
    stage('Package') {
      steps {
        ...
        bat "del *.zip"
        zip zipFile: "demoaspnetwebapp-1.0.0.${env.BUILD_NUMBER}.zip",
        dir: "demoaspnetwebapp\\bin\\Debug\\netcoreapp3.1\\publish"
      }
    }
    ...
    stage('Deploy') {
      steps {
        echo "Deploying to stage environment for more tests!";
        unzip zipFile: "demoaspnetwebapp-1.0.0.${env.BUILD_NUMBER}.zip",
